u:26-07-02|rid:28567768699|run:54|monthly-canonical:#342(July,verified live run54,0 comments no maint engagement)|maint-silent~90d(last~03-29)
RUN54:CLOSE#238(lowest April dup)+transparency comment(aw_0Yktb0sr). Verified live:#342 July monthly=0 comments,no checkbox changes,HEAD unchanged. 21 April dups remain. 1/run update_issue spent closing #238. Did NOT refresh #342 (alternating cadence; run55 refreshes). MCP list/search return [] as usual; curl REST works(rate60,58 remaining).
RUN53:MONTH-ROLLOVER. Closed #316(June canonical)+comment+created #342 "[Test Improver] Monthly Activity 2026-07"(fresh Run History,carried pending actions). Verified 18 TI-PRs unchanged,22 April dups,#265 #214 open. HEAD 9ffb0f3.
RUN52:CLOSE#232+comment. RUN51:REFRESH#316. RUN50:CLOSE#228.
ROTATION:run55=REFRESH #342(prepend run54[closed#238]+run55 entries to Run History;remove #238 from bulk-close action,now 21 dups),then run56=close#240,alternate close-dup/refresh(1/run update_issue HARD cap).close dups lowest#first:240,244,246...
...
READ:MCP list/search/issue_read return [] MOST runs BUT public single-issue + list(?state=open&labels=testing / ?state=all&sort=updated) via curl works(run43-54). Check /rate_limit first(60/hr shared-IP unauth).
