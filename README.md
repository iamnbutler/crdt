...
**@iamnbutler/crdt** is a plain-text CRDT written entirely in TypeScript, with no runtime dependencies or WebAssembly. Its implementation and benchmark harness were developed in [crdt-lab](https://github.com/iamnbutler/crdt-lab).

**[Benchmark report and replica demo](https://nate.rip/crdt/)** · [Raw measurement history](https://github.com/iamnbutler/crdt/tree/benchmark-data) · [Design](docs/run-design.md)
...
GitHub Actions checks every change, records the full matrix on `main` and weekly, persists results to `benchmark-data`, and publishes GitHub Pages. The repository's Pages source must be **GitHub Actions**. This replaces the previous benchmark, site, and issue-generating workflows. Earlier results remain in the history branch.
